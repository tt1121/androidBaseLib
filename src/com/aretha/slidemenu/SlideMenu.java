...
public class SlideMenu extends ViewGroup {
	...
	private int mAnimMaxDuration = 500;
	
	private float mPrimaryMenuMoveScale = 0;
	...
	private boolean mIsCanScrollMenu;
	private boolean mIsDrawShadow;
	
	private boolean mHandleKeyback = true;
	...
	public SlideMenu(Context context, AttributeSet attrs, int defStyle) {
		...
		setIsCanScrollMenu(a.getBoolean(R.styleable.SlideMenu_isCanScrollMenu, false));
		setIsDrawShadow(a.getBoolean(R.styleable.SlideMenu_isShowShadow, true));
		setMaxDuration(a.getInt(R.styleable.SlideMenu_maxDuration, mAnimMaxDuration));
		setPrimaryMenuMoveScale(a.getFloat(R.styleable.SlideMenu_primaryMenuMoveScale, mPrimaryMenuMoveScale));
		setHandleKeyback(a.getBoolean(R.styleable.SlideMenu_handleKeyback, mHandleKeyback));
		a.recycle();
	...
	public void setIsCanScrollMenu(boolean b) {
		mIsCanScrollMenu = b;
	}
	
	public void setIsDrawShadow(boolean b) {
		mIsDrawShadow = b;
	}
	
	public void setMaxDuration(int duration) {
		mAnimMaxDuration = duration;
	}
	
	public void setPrimaryMenuMoveScale(float scale) {
		mPrimaryMenuMoveScale = scale;
	}
	
	public void setHandleKeyback(boolean handleKeyback) {
		mHandleKeyback = handleKeyback;
	}
	 ...
	 * @param isAnimated
	 */
	public void open(final boolean isSlideLeft, final boolean isAnimated) {
		if(getWindowToken() == null) {
			mDelayRunnable = new Runnable() {
				@Override
				public void run() {
					open(isSlideLeft, isAnimated);
				}
			};
			return;
		}
	 ...
	 * @param isAnimated
	 */
	public void close(final boolean isAnimated) {
		if(getWindowToken() == null) {
			mDelayRunnable = new Runnable() {
				@Override
				public void run() {
	...
	public void smoothScrollContentTo(int targetOffset, float velocity) {
		...
		duration = Math.min(duration, mAnimMaxDuration);
		mScroller.abortAnimation();
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (Math.abs(x - mPressedX) >= mTouchSlop 
					&& (mIsTapInContent || mIsCanScrollMenu) 
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int index = 0; index < count; index++) {
			...
			case LayoutParams.ROLE_PRIMARY_MENU:
				...
				int menuLeft = (int)(paddingLeft + (mCurrentContentOffset * mPrimaryMenuMoveScale)
						- (measureWidth*mPrimaryMenuMoveScale));
				int menuRight = menuLeft + measureWidth;
				child.layout(menuLeft, statusBarHeight + paddingTop,
						menuRight, 
